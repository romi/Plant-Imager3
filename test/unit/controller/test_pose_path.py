"""Unit tests for the scan path pose primitives and helper generators.

These tests verify the low-level building blocks in
:mod:`plantimager.controller.scanner.path` — the ``Pose`` and ``PathElement``
data types, and the ``circle``, ``line1d``, and ``line3d`` helper functions
that generate coordinate/pan sequences. The design checks arithmetic and
attribute behavior of the primitives, and the point counts, values, and
wrapping of the generated sequences.
"""
...
class TestPose(unittest.TestCase):
    """Tests for the Pose data type."""

    def test_pose_add(self):
        """Adding two poses sums each coordinate component."""
        a = Pose(1, 2, 3, 10, 20)
        b = Pose(5, 5, 5, 30, 40)
        c = a + b
    ...
    def test_pose_repr(self):
        """The repr includes the pose's coordinate values."""
        p = Pose(1, 2, 3, 4, 5)
        r = repr(p)
        self.assertIn("x: 1", r)
    ...
    def test_pose_attributes(self):
        """The default pose exposes the expected attribute names."""
        p = Pose()
        self.assertEqual(p.attributes(), ["x", "y", "z", "pan", "tilt"])

    def test_pathelement_default(self):
        """PathElement defaults to exact_pose unless explicitly disabled."""
...
class TestCircleHelper(unittest.TestCase):
    """Tests for the circle helper generator."""

    def test_circle_three_points(self):
        """A three-point circle starts at the given point and steps by 120 degrees."""
    ...
    def test_circle_pan_wraps(self):
        """Circle pan values wrap to stay within [0, 360)."""
    ...
    def test_circle_subtests(self):
        """A circle returns one point per requested count across several sizes."""
        for n in [3, 9, 36]:
...
class TestLineHelpers(unittest.TestCase):
    """Tests for the line1d and line3d helper generators."""

    def test_line1d(self):
        """line1d returns evenly spaced points including both endpoints."""
        self.assertEqual(line1d(0, 10, 5), [0.0, 2.5, 5.0, 7.5, 10.0])

    def test_line1d_two_points(self):
        """A two-point line returns just the start and stop values."""
        self.assertEqual(line1d(5, 15, 2), [5.0, 15.0])

    def test_line1d_n1_raises(self):
        """A single-point line raises ZeroDivisionError."""
        with self.assertRaises(ZeroDivisionError):
            line1d(0, 10, 1)

    def test_line3d(self):
        """line3d returns matching evenly spaced coordinates for each axis."""
    ...
    def test_line3d_delegates(self):
        """line3d endpoints match the requested start and stop coordinates."""
...
class TestPath(unittest.TestCase):
    """Tests for the Path list subclass."""

    def test_path_is_list(self):
        """Path behaves as a list that accepts PathElement items."""
        p = Path()
